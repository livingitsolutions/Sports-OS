...
export class PgCompetitionStructureMaterializer implements CompetitionStructureMaterializer{
 ...
 async findState(id:string){try{const rows=await this.sql<{competition_format_id:string;entrant_count:number;seed_finalized_at:Date|string|null}[]>`SELECT competition_format_id,entrant_count,seed_finalized_at FROM competition_format_materializations WHERE competition_format_id=${id}`;const x=rows[0];return{ok:true as const,value:x?{competitionFormatId:x.competition_format_id as never,entrantCount:Number(x.entrant_count),seedFinalizedAt:x.seed_finalized_at?new Date(x.seed_finalized_at).toISOString() as never:null}:null};}catch(e){return bad("unavailable",neutralDetail(e));}}
