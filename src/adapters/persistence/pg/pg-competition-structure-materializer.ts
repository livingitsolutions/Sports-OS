...
export class PgCompetitionStructureMaterializer implements CompetitionStructureMaterializer{
 ...
 async findState(id:string){try{const rows=await this.sql<{competition_format_id:string;entrant_count:number;seed_finalized_at:Date|string|null;participants_materialized_at:Date|string|null}[]>`SELECT competition_format_id,entrant_count,seed_finalized_at,participants_materialized_at FROM competition_format_materializations WHERE competition_format_id=${id}`;const x=rows[0];return{ok:true as const,value:x?{competitionFormatId:x.competition_format_id as never,entrantCount:Number(x.entrant_count),seedFinalizedAt:x.seed_finalized_at?new Date(x.seed_finalized_at).toISOString() as never:null,participantsMaterializedAt:x.participants_materialized_at?new Date(x.participants_materialized_at).toISOString() as never:null}:null};}catch(e){return bad("unavailable",neutralDetail(e));}}
 async materialize(i:Parameters<CompetitionStructureMaterializer["materialize"]>[0]){try{
  await this.sql.begin(async tx=>{await tx`INSERT INTO competition_format_materializations(competition_format_id,entrant_count,created_at) VALUES(${i.format.id},${i.entrantCount??2},${i.createdAt})`;
   ...
   for(const c of i.contests)await tx`INSERT INTO contests(id,stage_id,sequence,status,scheduled_at,created_at,updated_at,version,plan_ref) VALUES(${c.id},${c.stageId},${c.sequence},${c.status},${c.scheduledAt},${c.createdAt},${c.updatedAt},${c.version},${c.planRef??null})`;
