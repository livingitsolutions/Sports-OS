...
export class InMemoryCompetitionStructureMaterializer implements CompetitionStructureMaterializer{
 private claims=new Map<string,{entrantCount:number;seedFinalizedAt:string|null;participantsMaterializedAt:string|null}>();readonly stages=new Map<string,Stage>();readonly contests=new Map<string,Contest>();failContestInsert=false;
 ...
 async findState(id:string){const x=this.claims.get(id);return{ok:true as const,value:x?{competitionFormatId:id as never,entrantCount:x.entrantCount,seedFinalizedAt:x.seedFinalizedAt as never,participantsMaterializedAt:x.participantsMaterializedAt as never}:null};}
 async materialize(i:Parameters<CompetitionStructureMaterializer["materialize"]>[0]){if(this.claims.has(i.format.id))return{ok:false as const,error:{kind:"already_materialized" as const}};if(this.failContestInsert)return{ok:false as const,error:{kind:"unavailable" as const}};this.claims.set(i.format.id,{entrantCount:i.entrantCount,seedFinalizedAt:null,participantsMaterializedAt:null});for(const x of i.stages)this.stages.set(x.id,x);for(const x of i.contests)this.contests.set(x.id,x);return{ok:true as const,value:{stages:i.stages,contests:i.contests}};}
 ...
 markParticipantsMaterialized(id:string,at:string){const x=this.claims.get(id);if(!x||x.participantsMaterializedAt)return false;x.participantsMaterializedAt=at;return true;}
}
